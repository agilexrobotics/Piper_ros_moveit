...
def convert_endpose(endpose):
    """
    根据输入 endpose 的长度自动判断是欧拉角(6个值) 还是 四元数(7个值)
    - 如果是6个值 (x, y, z, roll, pitch, yaw)，则转换成四元数 [x, y, z, qw, qx, qy, qz]
    - 如果是7个值 (x, y, z, qw, qx, qy, qz)，则直接返回
    """
    if len(endpose) == 6:
...
def main():
    ...
    for i in range(10): 
        ...
        time.sleep(1)
        endpose_euler = [0.099091, 0.008422, 0.246447, -0.788906275193957, 1.4467732301481797, -0.7904945248132718]
        call_joint_moveit_ctrl_endpose(endpose_euler)  # 末端位置控制(欧拉角)
        time.sleep(1)
        endpose_quaternion = [0.099091, 0.008422, 0.246447, -0.09079689034052749, 0.7663049838381912, -0.02157924359457128, 0.6356625934370577]
